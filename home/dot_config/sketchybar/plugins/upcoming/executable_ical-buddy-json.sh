#!/usr/bin/env bash
#
# Taken from https://josh.fail/2022/dump-calendar-app-events-to-json/ and modified
...
# Returns now, to the exact second
date_now() {
  date +"%Y-%m-%d %H:%M:%S %z"
}

days_from_now_end_of_day() {
...
parse_events() {
  ...
  while read -r line; do
    local raw_title="" raw_time="" raw_notes="" title="" calendar="" \
      start_at="" end_at="" duration="" notes="" start_secs="" \
      end_secs=""
    ...
    if [[ "$raw_time" =~ ^[012][0-9]:[0-5][0-9]\ -\ [012][0-9]:[0-5][0-9]$ ]]; then
      ...
      start_secs="$(reformat_date "$current_date ${raw_time% - *}:00" "%s" "%F %T")"
      end_secs="$(reformat_date "$current_date ${raw_time#* - }:00" "%s" "%F %T")"
    fi
...
# $7 - event duration (M)
# $8 - start time epoch seconds
# $9 - end time epoch seconds
format_row() {
  local title="$1" \
    calendar="$2" \
    date="$3" \
  ...
  jq --null-input \
    ...
    --arg start_secs "$start_secs" \
    --arg end_at   "$end_at" \
    --arg end_secs "$end_secs" \
    ...
    '
      {
        ...
        start_secs: (if $start_secs == "" then 0 else $start_secs end) | tonumber,
        end_at: (if $end_at == "" then null else $end_at end),
        end_secs: (if $end_secs == "" then 0 else $end_secs end) | tonumber,
...
main() {
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      -u | --upcoming) mode=upcoming; shift ;;
      -r | --raw) raw=1; shift ;;
  ...
  case "$mode" in
    ...
    upcoming)
      start_at="$(date_now)"
      end_at="$(days_from_now_end_of_day 1)"
      ;;
  esac
